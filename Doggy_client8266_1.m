...
brea(i:i+9) = typecast(uint8(fread(t,[1,20],'uint8')),'uint16');
time(i:i+9) = typecast(uint8(fread(t,[1,40],'uint8')),'uint32');
...
hold off
Breath_rate = numel(pks)/time(end)*60*1000;S
